...
public class SecurityExpression extends SecurityExpressionRoot
  ...
  private OpenAEVPrincipal getUser() {
    return (OpenAEVPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
  }
  ...
  private boolean isUserHasBypass() {
    OpenAEVPrincipal principal = getUser();
