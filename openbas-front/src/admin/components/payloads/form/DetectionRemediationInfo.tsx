...
export interface Props { detectionRemediation?: DetectionRemediationForm }

const DetectionRemediationInfo = ({ detectionRemediation }: Props) => {
  const theme = useTheme();
  const { t } = useFormatter();

  if (!detectionRemediation?.content) return null;

  let label;
  let color;
  let icon;

  if (detectionRemediation.author_rule === 'AI_OUTDATED') {
    label = t('Payload changed since rule was edited');
    color = theme.palette.warning.main;
    icon = <CheckIndeterminateIcon fontSize="small" />;
  } else if (detectionRemediation.author_rule === 'AI') {
    label = t('Rules generated by AI');
    color = theme.palette.ai.main;
    icon = <LogoXtmOneIcon fontSize="small" />;
  } else if (detectionRemediation.author_rule === 'HUMAN') {
